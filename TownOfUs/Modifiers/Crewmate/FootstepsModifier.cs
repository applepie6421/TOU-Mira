...
public sealed class FootstepsModifier : BaseModifier
{
    ...
    public override void FixedUpdate()
    {
        var cantContinue = _currentSteps == null || Player.AmOwner ||
                           PlayerControl.LocalPlayer.AreAbilitiesBlockedByComms() ||
        ...
        if (CheckDistance)
        {
            ...
            var footstep = new GameObject("Footstep")
            {
                transform =
                {
                    ...
                    position = new Vector3(Player.transform.position.x, Player.transform.position.y - 0.3f * Player.GetAppearance().Size.y, Player.transform.position.z + 0.001f),
